export const pages = {
  github: 'https://github.com/NatashaPanchina/hooks-library',
  source: {
    useOutsideClick:
      'https://github.com/NatashaPanchina/hooks-library/blob/main/src/hooks/outsideClick/useOutsideClick.ts',
    useLocalStorage:
      'https://github.com/NatashaPanchina/hooks-library/blob/main/src/hooks/localStorage/useLocalStorage.ts',
    useFetch:
      'https://github.com/NatashaPanchina/hooks-library/blob/main/src/hooks/fetch/useFetch.ts',
    useToggle:
      'https://github.com/NatashaPanchina/hooks-library/blob/main/src/hooks/toggle/useToggle.ts',
    usePagination:
      'https://github.com/NatashaPanchina/hooks-library/blob/main/src/hooks/pagination/usePagination.ts',
  },
  home: '/',
  docs: '/docs',
  useOutsideClick: '/docs/useOutsideClick',
  useLocalStorage: '/docs/useLocalStorage',
  useFetch: '/docs/useFetch',
  useToggle: '/docs/useToggle',
  usePagination: '/docs/usePagination',
};
